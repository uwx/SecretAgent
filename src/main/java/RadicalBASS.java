...
public class RadicalBASS implements RadicalMusic {
    ...
    public static ByteArrayOutputStream streamNoSize(final InputStream zipinputstream, final int entrySize) throws IOException {
        final ByteArrayOutputStream bytes = new ByteArrayOutputStream(entrySize < 0 ? 8192 : entrySize);
        //HLogger.log(zipentry.getName() + "; size = " + zipentry.getSize());
    ...
    private void run() {
        ...
        if (file != null) {
            try (ZipInputStream z = new ZipInputStream(new FileInputStream(file))) {
                ...
                if (entrySize == -1) {
                    final ByteArrayOutputStream ubos = streamNoSize(z, entrySize);
                    song = ubos.toByteArray();
                    songSize = ubos.size();

                    //if (entrySize <= 0) HLogger.log("entry size is " + songSize);

                } else {
                    //if (entrySize <= 0) HLogger.log("entry size is " + entrySize);
    ...
    private boolean playFile() {
        ...
        BASS_ChannelSetAttribute(chan, BASS_ATTRIB_VOL, 0.3F);
